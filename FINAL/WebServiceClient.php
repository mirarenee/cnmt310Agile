...
class WebServiceClient {
  ...
  public function callDB($sql,$params = null,$records = "all",$s = true) {
    if ($this->db->dbCall($sql,$params,"all",true) === false) {
      error_log("Database error: " . $this->db->getDBError());
      $status = "error";
      $details = false;
    } else {
      $status = "noerror";
      $details = $this->db->getDBResult();
    }
    $this->prepResult($status,$details);
  }
